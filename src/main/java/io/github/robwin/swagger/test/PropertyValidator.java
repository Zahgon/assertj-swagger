...
public class PropertyValidator {
    ...
    private boolean shouldValidateRefProperty(Property expectedProperty) {
        return RefProperty.class.isAssignableFrom(expectedProperty.getClass()) && isAssertionEnabled(SwaggerAssertionType.REF_PROPERTIES);
    }

    private boolean shouldValidateArrayProperty(Property expectedProperty) {
        return ArrayProperty.class.isAssignableFrom(expectedProperty.getClass()) && isAssertionEnabled(SwaggerAssertionType.ARRAY_PROPERTIES);
    }

    private boolean shouldValidateByteArrayProperty(Property expectedProperty) {
        return ByteArrayProperty.class.isAssignableFrom(expectedProperty.getClass()) && isAssertionEnabled(SwaggerAssertionType.BYTE_ARRAY_PROPERTIES);
    }

    private boolean shouldValidateStringProperty(Property expectedProperty) {
        return StringProperty.class.isAssignableFrom(expectedProperty.getClass()) && isAssertionEnabled(SwaggerAssertionType.STRING_PROPERTIES);
    }
    ...
    private void validateBasicPropertyFeatures(Property actualProperty, Property expectedProperty, String message) {
        ...
        softAssertions.assertThat(actualProperty).as(message).isEqualToComparingOnlyGivenFields(expectedProperty, BASIC_PROPERTIES);
    }
